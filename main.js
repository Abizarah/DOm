...
function makeBahasa(arg) {
    for (i = 0; i < arg.length; i++) {
        arg[i].jumlahBahasa = arg[i].languages.length
    }
}

makeBahasa(countries_data)
...
let totalPop = sortArray(countries_data, 'population', 10)
let totalLang = sortArray(countries_data, 'jumlahBahasa', 10)
...
function forDiv(arg, opsi) {
    //block paragrahp
    let test = document.createElement('p')
    test.setAttribute('class', 'subjudul')
    if (opsi == 'jumlahBahasa') {
        test.textContent = '10 most multi languages country '
    } else {
        test.textContent = '10 most populated country in the world'
    }
    ...
    for (let i of arg) {
        ...
        nama.style.background = 'aqua'
        console.log(nama.nextElementSibling)
        nama.textContent = i['name']
        nama.setAttribute('class', 'class1')
        // nama.style.border = '1px solid'
        nama.style.textAlign = 'center'
        nama.style.fontSize = '25px'
        // nama.classList.add('list-item')
        document.body.appendChild(nama)

        //block data
        let coba = document.createElement('div')
        coba.className = `${opsi}`
        coba.textContent = i[`${opsi}`]
        coba.style.fontSize = '25px'
        coba.style.background = 'orange'
        coba.setAttribute('class', 'class2')
        coba.classList.add('list-item')
        document.body.appendChild(coba)

        // btn1.removeEventListener('click', forDiv(totalPop, 'p'))
    }

}

function countBahasa(satu) {
    tmps = []
    for (let i = 0; i < satu.length; i++) {
        tmps.push(satu[i].languages)
    } return tmps
}
countBahasa(countries_data)
...
console.log(ganda(tmps))

let btn1 = document.querySelector('.btn-1')
btn1.addEventListener('click', (e) => {
    forDiv(totalPop, 'population')
...
btn2.addEventListener('click', () => {
    forDiv(totalLang, 'jumlahBahasa')
})

// const arg = document.querySelectorAll('.close')
